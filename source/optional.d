...
struct Optional(T) {
    ...
    */
    ...
    /**
        map value inside optional

        Returns
            `Optional(f(value))`
    */
    Optional!U map(alias f, U = typeof(f(T.init)))() const {
        return this.empty ? no!U : some(f(this.front));
    }
...
unittest {
    ...
    foreach (T; AliasSeq!(Optional!int, const Optional!int, immutable Optional!int)) {
        ...
        assert(-a == some(-10));
        assert(+b == none);
        assert(a.map!(to!double) == some(10.0));
        assert(b.map!(to!double) == none);
        assert(a + 10 == some(20));
        assert(b + 10 == none);
        assert(a - 5 == some(5));
        assert(b - 5 == none);
